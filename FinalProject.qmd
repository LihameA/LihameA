...
```{r, error=FALSE, message=FALSE}
AidData <- try(read_excel("/Users/lihamearouna/Dropbox/PSCI3200/ADv3.xlsx", sheet = "GCDF_3.0"))

# Now df should not generate warnings for those columns anymore
...
Covariates 
Fized effects: Modeling people's opinions about Chinese debt (a binary variable, Q65C), as a function of covariates and accounting for regiohnal differences 
...
modelsummary(
  ...
  stars = TRUE,
  output = "markdown",
  title = "Logistic Regression: Predictors of Opinion on Chinese Debt",
  coef_map = c(
...
Excellent — you're aiming to explicitly define four groups that capture the intersection of:

Education level (Low vs. High)

Country-level debt exposure (Low vs. High)

This is a great way to illustrate cross-national variation in perception, especially in how access to information (via education) conditions responses to Chinese debt.
...
model_grouped <- glm(
  Q65C ~ edu_debt_group + employment_status + trust_president,
...
modelsummary(
  model_grouped,
  stars = TRUE,
  title = "Perception of Chinese Loans by Education × Debt Level Groups",
  coef_map = c(
    "edu_debt_groupLowEd_HighDebt" = "LowEd × HighDebt",
    "edu_debt_groupHighEd_LowDebt" = "HighEd × LowDebt",
    "edu_debt_groupHighEd_HighDebt" = "HighEd × HighDebt",
    "employment_statusLooking" = "Employment: Looking",
    "employment_statusPart-time" = "Employment: Part-time",
    "employment_statusFull-time" = "Employment: Full-time",
    "trust_presidentMedium" = "Trust in President: Medium",
